...
export default function Home() {
  ...
  const fetchModels = useCallback(
    async (apiKey?: string) => {
      ...
      try {
        ...
        if (models.length > 0) {
          const currentModel =
            localStorage.getItem('selectedModel') || UI_DEFAULT_MODEL
          ...
          toast({
            ...
            description:
              'Could not find any models. Check your API key in settings or the .env file.',
          })
        }
      } catch (error) {
        toast({
  ...
  const handleGenerateArchitecture = async () => {
    ...
    setFinalIssueURL('')
    try {
      const result = await runGenerateArchitecture(
        { prd },
  ...
  const researchSingleTask = useCallback(
    async (task: Task) => {
      ...
      try {
        ...
        setTasks(currentTasks =>
          currentTasks.map(t =>
            t.title === task.title ? { ...t, details: formattedDetails } : t
          )
        )
        ...
        setTasks(currentTasks =>
          currentTasks.map(t =>
            t.title === task.title ? { ...t, details: errorMessage } : t
          )
        )
        if (selectedTask?.title === task.title) {
    ...
    try {
      ...
      if (!initialTasks || initialTasks.length === 0) {
        toast({
          ...
          description:
            'The AI could not generate a task list. Try adjusting the PRD or architecture.',
        })
        setLoading(prev => ({ ...prev, tasks: false }))
        return
      }

      const tasksWithPlaceholders = initialTasks.map(t => ({
        ...t,
        details: 'Researching...',
      }))
      ...
      toast({
        title: 'Task research complete!',
        description: 'All tasks have been detailed.',
      })
    } catch (error) {
      toast({
        variant: 'destructive',
  ...
  const handleCreateIssue = async () => {
    ...
    try {
      if (
        ...
        !selectedRepo ||
        tasks.length === 0 ||
        selectedRepo === LOCAL_MODE_REPO_ID
      ) {
        toast({
          variant: 'destructive',
          title: 'Cannot Create Issues',
          description:
            'Please ensure you have selected a repository and generated tasks.',
        })
      ...
      toast({
        ...
        description:
          'The implementation plan and sub-tasks are in your repository.',
        action: (
  ...
  const isResearchFailed = (details: string) =>
    details.startsWith('Failed to research task')
          ...
          <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Configure your API keys and select your preferred AI model.
                  Your Google AI API Key from the .env file will be used if left
                  blank here.
                </DialogDescription>
              </DialogHeader>
              <Form {...form}>
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                        <div className="space-y-0.5">
                          ...
                          <FormDescription>
                            Generate tasks and implementation steps using
            ...
            <motion.div key="step1" {...cardAnimation}>
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Choose a repository to create issues in, or use local mode
                    to just export data.
                    {!githubToken &&
                      " You'll need to set your GitHub token to see your repos."}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {loading.repos ? (
              ...
              <motion.div key="step2" {...cardAnimation}>
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Paste your Product Requirements Document below. Before
                      generating, ensure your Google AI API key is set in the
                      settings or your environment.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <Textarea
              ...
              <motion.div key="step3" {...cardAnimation}>
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      The AI has generated the following architecture,
                      specifications, and file structure. Review and edit if
                      needed.
                    </CardDescription>
                  </CardHeader>
                  ...
                  <CardFooter className="flex justify-end gap-2">
                    ...
                    <Button
                      ...
                      disabled={loading.tasks || loading.researching}
                    >
                  ...
                  {tasks.length > 0 && (
                    <CardFooter className="flex justify-end gap-2">
                      ...
                      <Button
                        ...
                        title={
                          selectedRepo === LOCAL_MODE_REPO_ID
                            ? 'Cannot create issues in local mode'
                            : ''
                        }
                      >
                        {loading.issue ? (
                          ...
                          <>
                            <Github className="mr-2 h-4 w-4" /> Create GitHub
                            Issue
                          </>
                        )}
                      </Button>
              ...
              <motion.div key="step5" {...cardAnimation}>
                <Card className="bg-gradient-to-br from-primary/20 to-background">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Your implementation plan has been created as a GitHub
                      Issue.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
      ...
      <Dialog open={isTaskDetailOpen} onOpenChange={setIsTaskDetailOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              Review and edit the details for this task. This will be included
              in the GitHub issue.
            </DialogDescription>
          </DialogHeader>
